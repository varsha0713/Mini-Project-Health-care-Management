        ...
        <script>
            document.getElementById('viewReportBtn').addEventListener('click', function() {
                fetch('/get_pdf', {
                    ...
                    body: JSON.stringify({
                        patient_id: 'p101' // Replace with the actual patient ID
                    })
                })
...
def get_pdf():
    ...
    # Fetch PDF from the database based on patient ID
    with connection.cursor() as cursor:
        sql = "SELECT report_pdf FROM patient_record WHERE patient_id = %s"
        cursor.execute(sql, (patient_id,))
        result = cursor.fetchone()
        pdf_blob = result['report_pdf']  # Assuming the column name storing the PDF is 'pdf_blob'
...
@app.route('/doctor-data-upload', methods=['POST'])
def doctor_data_upload():
    Patient_ID = request.form['patient_id']
    bp = request.form['bp']
    sugar = request.form['sugar']
    bmi = request.form['bmi']
    report_pdf = request.files['upload_report']
    
    # Check if all required data is provided
    if not all([Patient_ID, bp, sugar, bmi, report_pdf]):
        return 'Please provide all required data', 400
    
    # Save the uploaded file
    report_filename = report_pdf.filename
    report_pdf.save(report_filename)
    
    # Insert data into database
    cur = mysql.connection.cursor()
    cur.execute("INSERT INTO patient_report (Patient_ID, bp, sugar, bmi, report_pdf) VALUES (%s, %s, %s, %s, %s)", (Patient_ID, bp, sugar, bmi, report_filename))
    mysql.connection.commit()
    cur.close()
